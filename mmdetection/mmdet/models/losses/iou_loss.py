...
class DarkLoss(nn.Module):
    ...
    def __init__(self, eps=1e-6, reduction='mean', loss_weight=1.0, gamma=2, constant=12.8):
    ...
    def forward(self,
        ...
        # 筛选条件
        mask_tiny = (width <= 6.0) & (height <= 7.2)

        targets_tiny = target[mask_tiny]
        preds_tiny = pred[mask_tiny]
        targets_other = target[~mask_tiny]
        preds_other = pred[~mask_tiny]

        # print('===mask===')
        # print(len(preds_tiny))
        # print(len(targets_tiny))
        # print('===other===')
        # print(len(preds_other))
        # print(len(targets_other))

        #融合两种loss
        Wasserstein_loss = self.loss_weight * wasserstein_loss(
                preds_tiny,
                targets_tiny,
                weight,
                eps=self.eps,
                ...
                constant=self.constant,
                **kwargs)
        Ciou_loss = self.loss_weight * ciou_loss(
            preds_other,
            targets_other,
            weight,
            eps=self.eps,
            ...
            avg_factor=avg_factor,
            **kwargs)
        loss = Wasserstein_loss+Ciou_loss
        return loss
