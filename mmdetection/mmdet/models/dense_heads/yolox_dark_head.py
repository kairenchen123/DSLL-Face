...
class InverseIntegral(nn.Module):
    def __init__(self, reg_max=16):
        super(InverseIntegral, self).__init__()
        self.reg_max = reg_max
        self.register_buffer('project',
                             torch.linspace(0, self.reg_max, self.reg_max + 1))
    ...
    def forward(self, x):
        # x = x.reshape(-1, 1)
        # x = F.linear(x, self.project.type_as(x).unsqueeze(0).t())
        linear = nn.Linear(x.shape[1], 4*(self.reg_max+1)).to(device=x.device)
        relu = nn.ReLU()
        x = linear(x)
        x = relu(x)
        return x
...
class DARKHead(BaseDenseHead):
    ...
    def __init__(
        ...
        self.loss_dfl: nn.Module = MODELS.build(loss_dfl)
        ...
        self.inverse_integral = InverseIntegral(self.reg_max)
    ...
    def _init_layers(self) -> None:
        ...
        self.multi_level_reg_conf = nn.ModuleList()
        self.multi_level_gfl_reg = nn.ModuleList()
        self.scales = nn.ModuleList()
        for _ in self.strides:
            self.multi_level_cls_convs.append(self._build_stacked_convs())
            self.multi_level_reg_convs.append(self._build_stacked_convs())
            conv_cls, conv_reg, conv_obj, gfl_reg = self._build_predictor()
            reg_conf = self._build_conf()
            self.multi_level_conv_cls.append(conv_cls)
            self.multi_level_conv_reg.append(conv_reg)
            self.multi_level_conv_obj.append(conv_obj)
            self.multi_level_gfl_reg.append(gfl_reg)
            self.multi_level_reg_conf.append(reg_conf)
    ...
    def _build_predictor(self) -> Tuple[nn.Module, nn.Module, nn.Module, nn.Module]:
        ...
        gfl_reg = nn.Conv2d(self.feat_channels, 4 * (self.reg_max + 1), 3, padding=1)
        conv_obj = nn.Conv2d(self.feat_channels, 1, 1)
        return conv_cls, conv_reg, conv_obj, gfl_reg
    ...
    def init_weights(self) -> None:
        ...
        super(DARKHead, self).init_weights()
    ...
    def forward_single(self, x: Tensor, cls_convs: nn.Module,
                       ...
                       conv_reg: nn.Module,
                       conv_obj: nn.Module,
                       gfl_reg: nn.Module, reg_conf: nn.Module,
                       scale) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
        ...
        gfl_pred = scale(gfl_reg(reg_feat)).float()
        N, C, H, W = gfl_pred.size()
        prob = F.softmax(gfl_pred.reshape(N, 4, self.reg_max+1, H, W), dim=2)
        ...
        quality_score = reg_conf(stat.reshape(N, -1, H, W))
        cls_score = cls_score * quality_score

        return cls_score, bbox_pred, objectness, gfl_pred
    ...
    def forward(self, x: Tuple[Tensor]) -> Tuple[List]:
        ...
        """
        # multi_apply：此函数将“func”应用于多个输入，并将“func”的多个输出映射到不同的列表中。每个列表都包含对应于不同输入的相同类型的输出。
        return multi_apply(self.forward_single, x, self.multi_level_cls_convs,
                           self.multi_level_reg_convs,
                           self.multi_level_conv_cls,
                           self.multi_level_conv_reg,
                           self.multi_level_conv_obj,
                           self.multi_level_gfl_reg,
                           self.multi_level_reg_conf,
                           self.scales)
    ...
    def _bbox_post_process(self,
        ...
        the nms operation. Usually `with_nms` is False is used for aug test.1
    ...
    def loss_by_feat(
        ...
        flatten_gfl_preds = [
            gfl_pred.permute(0, 2, 3, 1).reshape(num_imgs, -1, 4 * (self.reg_max + 1))
            for gfl_pred in gfl_preds
        ]

        flatten_cls_preds = torch.cat(flatten_cls_preds, dim=1)
        flatten_bbox_preds = torch.cat(flatten_bbox_preds, dim=1)
        flatten_objectness = torch.cat(flatten_objectness, dim=1)
        flatten_gfl_preds = torch.cat(flatten_gfl_preds, dim=1)
        flatten_priors = torch.cat(mlvl_priors)
        flatten_bboxes = self._bbox_decode(flatten_priors, flatten_bbox_preds)
        ...
        if num_pos > 0:
            ## 计算dfl_loss中的标签，也就是将预测的分布作为标签
            # 获得flatten_glf_preds中的正样本
            dfl_targets = flatten_gfl_preds.view(-1, 4 * (self.reg_max + 1))[pos_masks]
            dfl_targets = self.integral(dfl_targets)
            # 对距离进行截断处理，防止标签超出范围
            eps = 0.1
            dfl_targets[:, 0] = dfl_targets[:, 0].clamp(min=0, max=self.reg_max - eps).detach()  # 左
            dfl_targets[:, 1] = dfl_targets[:, 1].clamp(min=0, max=self.reg_max - eps).detach()  # 上
            dfl_targets[:, 2] = dfl_targets[:, 2].clamp(min=0, max=self.reg_max - eps).detach()  # 右
            dfl_targets[:, 3] = dfl_targets[:, 3].clamp(min=0, max=self.reg_max - eps).detach()  # 下
            dfl_targets = dfl_targets.reshape(-1)

            ## 计算预测，将预测的bbox和gt的center进行求距离，从而得到预测距离值
            # 获得flatten_bboxes中的正样本
            pos_flatten_bboxes = flatten_bboxes.view(-1, 4)[pos_masks]
            # 计算gt的中心点坐标
            bbox_targets_center = self._get_gt_center(bbox_targets)
            # 将预测的flatten_bboxes与gt中心结合，这里得到的是其上下左右的距离
            dfl_preds = bbox2distance(bbox_targets_center, pos_flatten_bboxes, self.reg_max)
            dfl_preds = self.inverse_integral(dfl_preds).reshape(-1, self.reg_max+1)

            # weight_targets = flatten_cls_preds.view(-1, self.num_classes)[pos_masks].detach()
            ...
            loss_dfl = flatten_gfl_preds.sum() * 0

        loss_dict = dict(
            loss_cls=loss_cls, loss_bbox=loss_bbox, loss_obj=loss_obj, loss_dfl=loss_dfl)
    ...
    def _get_gt_center(self,
                    gt_bbox: Tensor) -> Tensor:
        gt_cx = (gt_bbox[:, 0] + gt_bbox[:, 2]) / 2
        gt_cy = (gt_bbox[:, 1] + gt_bbox[:, 3]) / 2
        return torch.stack([gt_cx, gt_cy], dim=-1)
