...
class XMLDataset(BaseDetDataset):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        # 判断图片格式
        dir = '/mnt/data2/ckr/mmdetection/data/VOCdevkit/VOC2007/JPEGImages'
        files = os.listdir(dir)

        for file in files:
            if file.lower().endswith(('.jpg', '.jpeg', '.png', '.gif', '.bmp')):
                first_image_path = os.path.join(dir, file)
                break
        ...
        for img_id in img_ids:
            file_name = osp.join(self.img_subdir, f'{img_id}.{file_extension}')
